# Source: https://github.com/jindongwang/Pytorch-CapsuleNet/blob/master/capsnet.py
import torch
...
class CapsNet(ModelWithLoss):
    ...
    def loss(self, data, x, target, reconstructions, CEL_for_classifier=True):
        classification_loss = (
            self.margin_loss(x, target)
            if not CEL_for_classifier
            else self.cross_entropy_loss(x, target)
        )
