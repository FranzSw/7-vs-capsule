...
def plot_confusion_matrix(dataloaders, class_names, model, suf=""):
    ...
    disp = ConfusionMatrixDisplay(
        confusion_matrix=confusion_matrix(y_true, y_pred), display_labels=class_names
    )
    disp.plot()
    plt.show()
...
def create_splitted_dataset():
    ...
    trainSet, valSet = DS(
        post_normalize_transform=preprocess,
        exclude_classes=[DS.all_tumor_class_names[1], DS.all_tumor_class_names[2]],
    ).subject_split(test_size=0.2)
...
def create_dataloaders(trainSet: Subset, valSet: Subset):
    return {
        ...
        "val": torch.utils.data.DataLoader(
            valSet, batch_size=128, shuffle=True, num_workers=8
        ),
    }
...
def train_model(
    dataloaders, class_names, model, criterion, optimizer, scheduler, num_epochs=25
):
    since = time.time()
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ["train", "val"]:
            ...
            # Iterate over data.
            for idx, (inputs, labels, bounding_boxes) in tqdm(
                enumerate(dataloaders[phase], 0), unit="batch", total=number_of_batches
            ):
...
def main():
    ...
    if resnet_config.weighted_loss:
        criterion = nn.CrossEntropyLoss(
            weight=trainSet.dataset.get_class_weights_inverse().to(device)
        )
    else:
        criterion = nn.CrossEntropyLoss()
