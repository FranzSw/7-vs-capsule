...
def compare_images(images: list[tuple[torch.Tensor, torch.Tensor, torch.Tensor]], predictions: list[torch.Tensor], actuals: list[torch.Tensor]):
    ...
    for i, (imageA, imageB, imageC) in enumerate(images):
        imgAnp, imgBnp, imgCnp = imageA.cpu().numpy().transpose((1, 2, 0)), imageB.cpu(
        ).numpy().transpose((1, 2, 0)), imageC.cpu().numpy().transpose((1, 2, 0))
