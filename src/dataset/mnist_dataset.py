...
class MNISTDataset(BaseDataset):
    ...
    def __init__(self, image_width=28, color_channels=3):
        ...
        transform = Compose(
            [
                ...
                Grayscale(color_channels),
                ToTensor(),
                Normalize(
                    (0.1307, 0.1307, 0.1307) if color_channels == 3 else (0.1307,),
                    (0.3081, 0.3081, 0.3081) if color_channels == 3 else (0.3081,),
                ),
            ]
        )
    ...
    def split(self, _, mode: Literal["subject", "random"] = "subject"):
        if mode != "random":
            print(
                f"WARNING: split mode {mode} not supported for MNIST dataset. Falling back to random splitting."
            )
