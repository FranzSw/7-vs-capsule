...
import numpy as np
import math
from capsnet_config import EM_Config

from loss.spread_loss import SpreadLoss
...
class PrimaryCaps(nn.Module):
    ...
    def __init__(self, A=32, B=32, K=1, P=4, stride=1):
        ...
        self.a = nn.Conv2d(
            in_channels=A, out_channels=B, kernel_size=K, stride=stride, bias=True
        )
...
class ConvCaps(nn.Module):
    ...
    def add_pathes(self, x, B, K, psize, stride):
        ...
        oh = ow = int(
            ((h - K) / stride) + 1
        )  # moein - changed from: oh = ow = int((h - K + 1) / stride)
        idxs = [
            [(h_idx + k_idx) for k_idx in range(0, K)]
